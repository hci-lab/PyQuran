...
class Testing_quran(unittest.TestCase):
    ...
    def test_fetch_aya(self):
        ...
        self.assertEqual(quran.fetch_aya(3, 74), aya)

        aya = 'الم'
        self.assertEqual(quran.fetch_aya(29, 1), aya)


        self.assertEqual(len(quran.fetch_aya(28, 88)), 82)

        aya = 'الر تلك ءايت الكتب الحكيم'
        self.assertEqual(quran.fetch_aya(10, 1), aya)

        aya = 'واتبع ما يوحى إليك واصبر حتى يحكم الله وهو خير الحكمين'
        self.assertEqual(quran.fetch_aya(10, 109), aya)

        self.assertEqual(len(quran.fetch_aya(6, 165)), 105)

        aya = 'الحمد لله رب العلمين'
        self.assertEqual(quran.fetch_aya(1, 2), aya)
